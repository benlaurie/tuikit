...
lazy_static! {
    static ref TERM: Arc<Term> = Arc::new(initialize_term());
}
...
impl Term {
    ...
    fn calc_preferred_height(&self, prefer: &TermHeight, height: usize) -> usize {
        match *prefer {
            ...
            TermHeight::Percent(p) => {
                max(MIN_HEIGHT, height * min(p, 100) / 100)
            },
        }
    }
    ...
    fn resize(&self, screen_width: usize, screen_height: usize) -> Result<()> {
        ...
        let mut screen = self.screen.lock().expect("termbox:resize failed to get screen");
    ...
    pub fn present(&self) -> Result<()> {
        ...
        let mut commands = self.screen.lock().expect("termbox:present failed to get screen").present();
        let mut mutex_output = self.output.lock()
        ...
        for cmd in commands.iter_mut() {
            if let Command::CursorGoto{row, col} = *cmd {
                *cmd = Command::CursorGoto {row: row + cursor_row, col}
            }
        }
    ...
    pub fn pause(&self) -> Result<()> {
        ...
        let _ = self.output.lock().unwrap().take();
        Ok(())
    }

    fn get_cursor_pos(&self, keyboard: &mut KeyBoard, output: &mut Output)
        -> Result<(usize, usize)> {
        output.ask_for_cpr();
    ...
    fn ensure_height(&self, mut keyboard: &mut KeyBoard, mut output: &mut Output) {
        ...
        // initialize
        let (screen_width, screen_height) = output.terminal_size()
        ...
        if height_to_be >= screen_height {
            ...
            for _ in 0..(cursor_row + height_to_be - screen_height) {
                output.write("\n");
            }
            state.bottom_intact = true;
